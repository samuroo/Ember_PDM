...
def draw_box3d(ax, box, color="orange", linewidth=1.2):
    ...
    corners = [                     # List all 8 vertices of box
        (xs[0], ys[0], zs[0]),
        (xs[1], ys[0], zs[0]),
        (xs[1], ys[1], zs[0]),
        (xs[0], ys[1], zs[0]),
    ...
    edges = [                       # List of edges made from vertex pairs
...
def solve_rrt_from_urdf(
        ...
        # Convert URDF to AABB for each detected box
        ...
        box_obstacles = [BoxObstacle3D(*b) for b in boxes_raw]
        ...
        # Setup Matplotlib visulaizer (optional)
        ax = None
        plt = None
        ...
        # Live drawing callback function (optional, part of visulaizer)
        def draw_edge(p1, p2):
            if not visualize:
                return
            x = [p1[0], p2[0]]
        ...
        if cfg is None:
            ...
            if algo_name == "bit_star":
                cfg = BITStarConfig( batch_size=200,max_batches=5, neighbor_radius=5,
